...
from air import urls
from air.coders import Coder
from air.dump import dump  # noqa: F401
from air.io import InputOutput
from air.main import main as cli_main
from air.scrape import Scraper, has_playwright
...
def search(text=None):
    results = []
    for root, _, files in os.walk("air"):
        for file in files:
...
class GUI:
    ...
    def show_edit_info(self, edit):
        ...
        if commit_hash:
            ...
            if commit_hash == self.coder.last_air_commit_hash:
                show_undo = True
    ...
    def do_sidebar(self):
        with st.sidebar:
            ...
            st.warning(
                "This browser version of air is experimental. Please share feedback in [GitHub"
                " issues](https://github.com/Air-AI/air/issues)."
            )
    ...
    def do_recommended_actions(self):
        text = "Air works best when your code is stored in a git repo.  \n"
        ...
        with st.expander("Recommended actions", expanded=True):
            ...
            with st.popover("Update your `.gitignore` file"):
                st.write("It's best to keep air's internal files out of your git repo.")
                self.button("Add `.air*` to `.gitignore`", key=random.random(), help="?")
    ...
    def do_add_files(self):
        fnames = st.multiselect(
            ...
            help=(
                ...
                " on. Air will pull in other relevant code to provide context to the LLM."
            ),
        )
    ...
    def initialize_state(self):
        ...
        self.state.init("last_air_commit_hash", self.coder.last_air_commit_hash)
    ...
    def process_chat(self):
        ...
        with self.messages:
            edit = dict(
                role="edit",
                fnames=self.coder.air_edited_files,
            )
            if self.state.last_air_commit_hash != self.coder.last_air_commit_hash:
                edit["commit_hash"] = self.coder.last_air_commit_hash
                edit["commit_message"] = self.coder.last_air_commit_message
                commits = f"{self.coder.last_air_commit_hash}~1"
                diff = self.coder.repo.diff_commits(
                    self.coder.pretty,
                    commits,
                    self.coder.last_air_commit_hash,
                )
                edit["diff"] = diff
                self.state.last_air_commit_hash = self.coder.last_air_commit_hash
    ...
    def do_undo(self, commit_hash):
        ...
        if (
            self.state.last_air_commit_hash != commit_hash
            or self.coder.last_air_commit_hash != commit_hash
        ):
...
def gui_main():
    st.set_page_config(
        ...
        menu_items={
            ...
            "Report a bug": "https://github.com/Air-AI/air/issues",

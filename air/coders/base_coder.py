...
from air import __version__, models, prompts, urls, utils
from air.analytics import Analytics
from air.commands import Commands
from air.exceptions import LiteLLMExceptions
from air.history import ChatSummary
from air.io import ConfirmGroup, InputOutput
from air.linter import Linter
from air.llm import litellm
from air.models import RETRY_TIMEOUT
from air.reasoning_tags import (
    REASONING_TAG,
...
from air.repomap import RepoMap
from air.run_cmd import run_cmd
from air.utils import format_content, format_messages, format_tokens, is_image_file
from air.waiting import WaitingSpinner
...
class Coder:
    ...
    repo = None
    last_air_commit_hash = None
    air_edited_files = None
        ...
        **kwargs,
        ...
        if from_coder:
            ...
            update = dict(
                ...
                air_commit_hashes=from_coder.air_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"Air v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .airignore"
                )
    ...
    def __init__(
        ...
        if air_commit_hashes:
            self.air_commit_hashes = air_commit_hashes
        else:
            self.air_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches airignore spec.")
                continue
    ...
    def send_message(self, inp):
        ...
        if edited:
            self.air_edited_files.update(edited)
    ...
    def auto_commit(self, edited, context=None):
        ...
        try:
            res = self.repo.commit(fnames=edited, context=context, air_edits=True, coder=self)
            if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_air_commit_hash = commit_hash
        self.air_commit_hashes.add(commit_hash)
        self.last_air_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head_commit_sha():
            self.io.tool_output("You can use /undo to undo and discard each air commit.")

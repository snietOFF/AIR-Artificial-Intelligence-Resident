...
VERSION_CHECK_FNAME = Path.home() / ".air" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of air from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of air from the main branch?",
        ["git+https://github.com/Air-AI/air.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of air from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer air version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of air?"
...
"""
    ...
    if success:
        io.tool_output("Re-run air to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/air-chat/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = air.__version__

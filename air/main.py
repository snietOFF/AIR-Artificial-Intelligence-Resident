...
from air import __version__, models, urls, utils
from air.analytics import Analytics
from air.args import get_parser
from air.coders import Coder
from air.coders.base_coder import UnknownEditFormat
from air.commands import Commands, SwitchCoder
from air.copypaste import ClipboardWatcher
from air.deprecated import handle_deprecated_model_args
from air.format_settings import format_settings, scrub_sensitive_info
from air.history import ChatSummary
from air.io import InputOutput
from air.llm import litellm  # noqa: F401; properly init litellm on launch
from air.models import ModelSettings
from air.onboarding import offer_openrouter_oauth, select_default_model
from air.repo import ANY_GIT_ERROR, GitRepo
from air.report import report_uncaught_exceptions
from air.versioncheck import check_version, install_from_main_branch, install_upgrade
from air.watch import FileWatcher
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run air in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track air's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".air"):
            patterns_to_add.append(".air*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the air browser feature",
        ["air-chat[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/Air-AI/air/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/Air-AI/air/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".air.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading air model settings: {e}")
        return 1
...
def load_dotenv_files(git_root, dotenv_fname, encoding="utf-8"):
    ...
    oauth_keys_file = Path.home() / ".air" / "oauth-keys.env"
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("air.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".air.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("Air only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run air --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            io.tool_output(
                "Air respects your privacy and never collects your code, chat messages, keys or"
                " personal info."
            )
            io.tool_output(f"For more info: {urls.analytics}")
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve air?"
            )
    ...
    if args.apply:
        ...
        # For testing #2879
        # from air.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".air" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install air properly?")

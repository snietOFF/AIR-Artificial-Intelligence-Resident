...
class GitRepo:
    repo = None
    air_ignore_file = None
    air_ignore_spec = None
    air_ignore_ts = 0
    air_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if air_ignore_file:
            self.air_ignore_file = Path(air_ignore_file)

    def commit(self, fnames=None, context=None, message=None, air_edits=False, coder=None):
        """
        ...
        Args:
            ...
            air_edits (bool, optional): Whether the changes were made by Air. Defaults to False.
        ...
        Air generated the changes (`air_edits`).
        ...
        - air_edits=True: Changes were generated by Air (LLM).
        - air_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (air)".
        - --attribute-committer: Modify Committer name to "User Name (air)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: air (<model>) <air@air.chat>" trailer to commit message.

        Behavior Summary:

        1. When air_edits = True (AI Changes):
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(air), Committer=You, Trailer added
        - User commit (defaults): air_edits=False -> Author=You, Committer=You(air)
        - User commit with explicit no-committer: air_edits=False,
        ...
        prefix_commit_message = air_edits and (
        ...
        if air_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: air ({model_name}) <air@air.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to air edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            air_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of air_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an air edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (air_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "air: " + commit_message
        ...
        committer_name = f"{original_user_name} (air)"
    ...
    def refresh_air_ignore(self):
        if not self.air_ignore_file:
            return

        current_time = time.time()
        if current_time - self.air_ignore_last_check < 1:
            return

        self.air_ignore_last_check = current_time

        if not self.air_ignore_file.is_file():
            return

        mtime = self.air_ignore_file.stat().st_mtime
        if mtime != self.air_ignore_ts:
            self.air_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.air_ignore_file.read_text().splitlines()
            self.air_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.air_ignore_file or not self.air_ignore_file.is_file():
            return False
        ...
        return self.air_ignore_spec.match_file(fname)
